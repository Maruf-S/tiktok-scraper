...
class Downloader {
    constructor({ progress, proxy, retry, noWaterMark, headers, filepath, bulk, cookieJar }) {
    ...
    toBuffer(item) {
        ...
        debug_1.default('tiktok-scraper:Downloader:toBuffer')(url);
        return p_retry_1.default(() => new Promise((resolve, reject) => {
    ...
    async downloadSingleVideo(post) {
        ...
        debug_1.default('tiktok-scraper:Downloader:downloadSingleVideo')(url);
        ...
        const result = await p_retry_1.default(() => request_promise_1.default(options), {
            retries: this.retry,
            onFailedAttempt: error => {
                console.error(error);
                console.log(`\nAttempt ${error.attemptNumber} (of ${error.attemptNumber + error.retriesLeft}) to download ${url} has failed.`);
