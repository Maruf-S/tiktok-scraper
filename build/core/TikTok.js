...
const tiktok_signature_1 = __importDefault(require("tiktok-signature"));
...
class TikTokScraper extends events_1.EventEmitter {
    constructor({ download, filepath, filetype, proxy, strictSSL = true, asyncDownload, cli = false, event = false, progress = false, input, number, since, type, by_user_id = false, store_history = false, historyPath = '', noWaterMark = false, useTestEndpoints = false, fileName = '', timeout = 0, retry = 3, bulk = false, zip = false, test = false, hdVideo = false, webHookUrl = '', method = 'POST', headers, sessionList = [], throttleLimit, throttleInterval, }) {
        ...
        if (this.throttle) {
            debug_1.default('tiktok-scraper:TikTokScraper')(`Setting up a throttle limit of '${throttleLimit}' request for every ${throttleInterval}ms`);
    ...
    request({ uri, method, qs, body, form, headers, json, gzip, followAllRedirects, simple = true }, bodyOnly = true) {
        debug_1.default('tiktok-scraper:request')(method, uri, JSON.stringify({ qs, body, form }));
        return p_retry_1.default(() => new Promise(async (resolve, reject) => {
    ...
    async sign(url) {
        const signer = new tiktok_signature_1.default(undefined, this.headers['user-agent']);
        await signer.init();
        const auth = await signer.sign(url);
        await signer.close();
        debug_1.default('tiktok-scraper:sign')(auth.signed_url);
    ...
    async extractVideoId(item) {
        ...
        debug_1.default('tiktok-scraper:extractVideoId')(`Fetching video id of post ${item.id}`);
        try {
    ...
    async getUrlWithoutTheWatermark(uri) {
        ...
        debug_1.default('tiktok-scraper:getUrlWithoutTheWatermark')(`Obtaining url without watermark`);
        const options = {
            uri,
            method: 'GET',
            headers: {
    ...
    mainLoop() {
        return new Promise((resolve, reject) => {
            ...
            async_1.forEachLimit(taskArray, this.asyncScraping(), (item, cb) => {
                debug_1.default('tiktok-scraper:mainLoop')(`Iterating on '${this.scrapeType}' - Step #${item}`);
                switch (this.scrapeType) {
                    case 'user':
                        this.getUserId()
    ...
    async submitScrapingRequest(query, updatedApiResponse = false) {
        debug_1.default('tiktok-scraper:submitScrapingRequest')(query);
        ...
        debug_1.default('tiktok-scraper:submitScrapingRequest')(`${(updatedApiResponse ? result.itemList : result.items).length} new posts have been fetched`);
        ...
        if (done) {
            debug_1.default('tiktok-scraper:submitScrapingRequest')(`Done fetching posts`);
            return true;
        }
        debug_1.default('tiktok-scraper:submitScrapingRequest')(`Not done yet`);
